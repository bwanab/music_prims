defmodule NoteTest do
  ...
  describe "Note.new/2" do
    ...
    test "creates a note with both duration and velocity specified" do
      note = Note.new(:D, octave: 3, duration: 8, velocity: 80)
      assert note.note == :D
    ...
    test "creates a note with duration but no velocity, defaults velocity to 100" do
      note = Note.new(:E, octave: 5, duration: 2)
      assert note.note == :E
  ...
  describe "MidiNote.note_to_midi/1" do
    test "converts middle C (C4) to MIDI note 60" do
      note = Note.new(:C, octave: 4, duration: 4, velocity: 100)
    ...
    test "converts C5 to MIDI note 72" do
      note = Note.new(:C, octave: 5, duration: 2, velocity: 80)
      midi = MidiNote.note_to_midi(note)
    ...
    test "converts C#4 to MIDI note 61" do
      note = Note.new(:C!, octave: 4, duration: 8, velocity: 90)
      midi = MidiNote.note_to_midi(note)
    ...
    test "converts A0 to MIDI note 21 (lowest piano note)" do
      note = Note.new(:A, octave: 0, duration: 4, velocity: 100)
      midi = MidiNote.note_to_midi(note)
    ...
    test "converts C8 to MIDI note 108 (high piano note)" do
      note = Note.new(:C, octave: 8, duration: 4, velocity: 100)
      midi = MidiNote.note_to_midi(note)
  ...
  describe "MidiNote.get_lily_duration/2" do
    test "plain half note" do
      note = Note.new(:C, octave: 3, duration: 2)
      lily = MidiNote.get_lily_duration(Sonority.duration(note), 4)
      assert lily == "2"
    end
    test "plain whole note" do
      note = Note.new(:C, octave: 3, duration: 4)
      lily = MidiNote.get_lily_duration(Sonority.duration(note), 4)
      assert lily == "1"
    end
    test "plain quarter note" do
      note = Note.new(:C, octave: 3, duration: 1)
      lily = MidiNote.get_lily_duration(Sonority.duration(note), 4)
      assert lily == "4"
    end
    test "plain eighth note" do
      note = Note.new(:C, octave: 3, duration: 0.5)
      lily = MidiNote.get_lily_duration(Sonority.duration(note), 4)
      assert lily == "8"
    end
    test "dotted half note" do
      note = Note.new(:C, octave: 3, duration: 3)
      lily = MidiNote.get_lily_duration(Sonority.duration(note), 4)
      assert lily == "2."
    end
    test "dotted quarter note" do
      note = Note.new(:C, octave: 3, duration: 1.5)
      lily = MidiNote.get_lily_duration(Sonority.duration(note), 4)
      assert lily == "4."
    end
    test "dotted eighth note" do
      note = Note.new(:C, octave: 3, duration: 0.75)
      lily = MidiNote.get_lily_duration(Sonority.duration(note), 4)
      assert lily == "8."
    end
    test "dotted 16th note" do
      note = Note.new(:C, octave: 3, duration: 0.375)
      lily = MidiNote.get_lily_duration(Sonority.duration(note), 4)
      assert lily == "16."
    end
    test "long plain note remainder" do
      note = Note.new(:C, octave: 3, duration: 9)
      lily = MidiNote.get_lily_duration(Sonority.duration(note), 4)
      assert lily == "4*9"
    end
    test "long plain note no remainder" do
      note = Note.new(:C, octave: 3, duration: 12)
      lily = MidiNote.get_lily_duration(Sonority.duration(note), 4)
      assert lily == "1*3"
    end
    test "long dotted note" do
      note = Note.new(:C, octave: 3, duration: 9.5)
      lily = MidiNote.get_lily_duration(Sonority.duration(note), 4)
      assert lily == "8*19"
    end
  ...
  describe "Note.enharmonic_equal?/2" do
    ...
    test "identifies enharmonic equivalence in Note structs" do
      c_sharp = Note.new(:C!, octave: 4)
      d_flat = Note.new(:Db, octave: 4)
      assert Note.enharmonic_equal?(c_sharp, d_flat)

      f_sharp = Note.new(:F!, octave: 3)
      g_flat = Note.new(:Gb, octave: 3)
      assert Note.enharmonic_equal?(f_sharp, g_flat)

      c4 = Note.new(:C, octave: 4)
      c5 = Note.new(:C, octave: 5)
